...
def _connect() -> None:
    ...
    logger.info("[loyalty_rabbitmq] Conectado al exchange '%s'", EXCHANGE_NAME)
...
def _get_channel():
    ...
    if not _is_connected():
        logger.warning(
            "[loyalty_rabbitmq] Canal no disponible — reconectando...")
        try:
...
def _build_message(event_type: str, data: dict) -> dict:
    return {
        ...
        "service_origin": "loyalty_service",
        "version":        "1.0",
        "data":           data,
    }
...
def publish_event(event_type: str, data: dict) -> None:
    ...
    try:
        ...
        logger.debug(
            "[loyalty_rabbitmq] Publicado '%s' | event_id: %s",
            event_type, message["event_id"],
        )

    except pika.exceptions.AMQPConnectionError as exc:
        logger.error(
            "[loyalty_rabbitmq] Conexión caída al publicar '%s': %s",
            event_type, exc,
        )
        global _connection, _channel
        _connection = None
        ...
        logger.error(
            "[loyalty_rabbitmq] Error inesperado al publicar '%s': %s",
            event_type, exc,
        )
