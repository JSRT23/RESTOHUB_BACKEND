...
def _connect() -> None:
    ...
    logger.info("[staff_rabbitmq] Conectado al exchange '%s'", EXCHANGE_NAME)
...
def _get_channel():
    ...
    if not _is_connected():
        logger.warning(
            "[staff_rabbitmq] Canal no disponible — reconectando...")
        try:
...
# La routing key es el event_type (ej: "app.staff.turno.creado").
# ---------------------------------------------------------------------------
...
def publish_event(event_type: str, data: dict) -> None:
    ...
    try:
        ...
        logger.debug("[staff_rabbitmq] Publicado '%s'", event_type)

    except pika.exceptions.AMQPConnectionError as exc:
        logger.error(
            "[staff_rabbitmq] Error de conexión publicando '%s': %s", event_type, exc)
    except Exception as exc:
        logger.error(
            "[staff_rabbitmq] Error publicando '%s': %s", event_type, exc)
