...
class CrearProveedor(graphene.Mutation):
    ...
    def mutate(self, info, nombre, pais, alcance=None, pais_destino=None,
        ...
        if _is_error(data):
            return CrearProveedor(ok=False, error=_extract_error(data, "Error al crear proveedor."))
        if not data:
...
class ActualizarProveedor(graphene.Mutation):
    ...
    def mutate(self, info, id, alcance=None, pais_destino=None,
        ...
        if _is_error(data):
            return ActualizarProveedor(ok=False, error=_extract_error(data, "Error al actualizar proveedor."))
        if not data:
...
class CrearAlmacen(graphene.Mutation):
    ...
    def mutate(self, info, restaurante_id, nombre, descripcion=None):
        ...
        if _is_error(data):
            return CrearAlmacen(ok=False, error=_extract_error(data, "Error al crear almacén."))
        if not data:
...
class RegistrarStock(graphene.Mutation):
    ...
    def mutate(self, info, ingrediente_id, nombre_ingrediente, almacen_id,
        ...
        if _is_error(data):
            return RegistrarStock(ok=False, error=_extract_error(data, "Error al registrar stock."))
        if not data:
...
class AjustarStock(graphene.Mutation):
    ...
    def mutate(self, info, id, cantidad, descripcion):
        ...
        if _is_error(data):
            return AjustarStock(ok=False, error=_extract_error(data, "Error al ajustar stock."))
        if not data:
...
class RegistrarLote(graphene.Mutation):
    ...
    def mutate(self, info, ingrediente_id, almacen_id, proveedor_id,
        ...
        if _is_error(data):
            return RegistrarLote(ok=False, error=_extract_error(data, "Error al registrar lote."))
        if not data:
...
class RetirarLote(graphene.Mutation):
    ...
    def mutate(self, info, id):
        ...
        if _is_error(data):
            return RetirarLote(ok=False, error=_extract_error(data, "Error al retirar lote."))
        if not data:
...
class CrearOrdenCompra(graphene.Mutation):
    ...
    def mutate(self, info, proveedor_id, moneda, detalles, **kwargs):
        ...
        if _is_error(data):
            return CrearOrdenCompra(ok=False, error=_extract_error(data, "Error al crear orden de compra."))
        if not data:
...
class EnviarOrdenCompra(graphene.Mutation):
    ...
    def mutate(self, info, id):
        ...
        if _is_error(data):
            return EnviarOrdenCompra(ok=False, error=_extract_error(data, "Error al enviar orden."))
        if not data:
            return EnviarOrdenCompra(ok=False, error="Sin respuesta del servidor.")
...
class RecibirOrdenCompra(graphene.Mutation):
    ...
    def mutate(self, info, id, detalles, notas=""):
        ...
        if _is_error(data):
            return RecibirOrdenCompra(ok=False, error=_extract_error(data, "Error al recibir orden."))
        if not data:
            return RecibirOrdenCompra(ok=False, error="Sin respuesta del servidor.")
...
class CancelarOrdenCompra(graphene.Mutation):
    ...
    def mutate(self, info, id):
        ...
        if _is_error(data):
            return CancelarOrdenCompra(ok=False, error=_extract_error(data, "Error al cancelar orden."))
        if not data:
            return CancelarOrdenCompra(ok=False, error="Sin respuesta del servidor.")
...
class ResolverAlerta(graphene.Mutation):
    ...
    def mutate(self, info, id):
        ...
        if _is_error(data):
            return ResolverAlerta(ok=False, error=_extract_error(data, "Error al resolver alerta."))
        if not data:
...
class IgnorarAlerta(graphene.Mutation):
    ...
    def mutate(self, info, id):
        ...
        if _is_error(data):
            return IgnorarAlerta(ok=False, error=_extract_error(data, "Error al ignorar alerta."))
        if not data:
