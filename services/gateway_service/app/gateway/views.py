...
class CrearPreferenciaView(View):
    ...
    def post(self, request):
        ...
        # Leer desde settings (que lee de env vars)
        ...
        success_url = f"{frontend_url}/pago-exitoso"
        failure_url = f"{frontend_url}/pago-fallido"
        pending_url = f"{frontend_url}/pago-exitoso"
    ...
    for item in items:
        ...
        <tr>
          ...
          <td style="padding:10px 0;border-bottom:1px solid #f0f0e8;text-align:right;
            ...
            {fmt(item.get('precio', 0) * item.get('cantidad', 1))} {moneda}
          </td>
        </tr>"""
...
<table width="100%" cellpadding="0" cellspacing="0" style="padding:40px 20px;">
  <tr><td align="center">
    <table width="520" cellpadding="0" cellspacing="0"
      ...
      <tr>
        <td style="padding:0 40px 20px;">
          ...
          <table width="100%" cellpadding="0" cellspacing="0">
            ...
            <!-- Total -->
            <tr>
              <td style="padding:14px 0 0;font-family:Georgia,serif;font-size:16px;
                  font-weight:700;color:#141410;">Total</td>
              <td style="padding:14px 0 0;text-align:right;font-family:Georgia,serif;
      ...
      <!-- Footer -->
      <tr>
        <td style="background:#f5f5ec;padding:18px 40px;border-top:1px solid rgba(0,0,0,.06);">
...
class EmailConfirmacionView(View):
    ...
    Envía email de resumen de compra al usuario via Gmail SMTP.
    """
    ...
    def post(self, request):
        ...
        # Leer config Gmail del settings
        host = getattr(settings, "EMAIL_HOST",          "smtp.gmail.com")
        port = getattr(settings, "EMAIL_PORT",          587)
        user = getattr(settings, "EMAIL_HOST_USER",     "")
        password = getattr(settings, "EMAIL_HOST_PASSWORD", "")
        from_str = getattr(settings, "EMAIL_FROM",
                           f"RestoHub <{user}>") if user else "RestoHub <noreply@restohub.com>"

        if not user or not password:
            logger.warning(
                "[email_confirmacion] EMAIL_HOST_USER/PASSWORD no configurados.")
            return JsonResponse({"ok": False, "error": "Email no configurado."}, status=500)
        ...
        msg["Subject"] = f"✅ Compra confirmada — RestoHub #{payment_id}"
        msg["From"] = from_str
        msg["To"] = email
        ...
        try:
            ...
            logger.info(f"[email_confirmacion] ✓ Email enviado a {email}")
            return JsonResponse({"ok": True})
        except Exception as exc:
            logger.error(f"[email_confirmacion] Error: {exc}")
