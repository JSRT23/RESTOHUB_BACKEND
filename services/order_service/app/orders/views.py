# order_service/app/orders/views.py
# FIX: entregar() ahora guarda total_cobrado si viene en el payload
...
from .serializers import (
    PedidoSerializer, PedidoListSerializer, PedidoWriteSerializer,
    PedidoCambioEstadoSerializer, DetallePedidoSerializer,
    DetallePedidoWriteSerializer, ComandaCocinaSerializer,
    ComandaCocinaWriteSerializer, SeguimientoPedidoSerializer,
    EntregaPedidoSerializer, EntregaPedidoWriteSerializer,
)
...
class PedidoViewSet(viewsets.ModelViewSet):
    ...
    def entregar(self, request, pk=None):
        ...
        total_cobrado = serializer.validated_data.get("total_cobrado")  # FIX
        ...
        # FIX: guardar total real cobrado (con descuentos aplicados)
        if total_cobrado is not None:
            pedido.total_cobrado = total_cobrado
            update_fields.append("total_cobrado")
    ...
    def detalles(self, request, pk=None):
        ...
        if request.method == "GET":
            return Response(DetallePedidoSerializer(pedido.detalles.all(), many=True).data)
        if pedido.estado != "RECIBIDO":
            return Response(
